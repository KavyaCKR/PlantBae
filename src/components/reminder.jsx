...
export default function Dashboard() {
  ...
  const handleFormSubmit = async (e) => {
    ...
    const response = await api.set_reminder(user.user.primaryEmailAddress.emailAddress, formData.task, formData.time);
  ...
  useEffect(() => {
    ...
    if (user && user.user && user.user.primaryEmailAddress) {
      ...
      async function get_data() {
        try {
          // Fetch schedule data
          const response = await api.get_reminder(user.user.primaryEmailAddress.emailAddress);
  ...
  }, [ formData, reminder, user]); // Use specific property as dependency
          ...
          <nav className="nav">
            <div className="nav-upper-options">
              ...
              <div className="nav-option option4">
                ...
                <a href="/remainder"><h3>Remainder</h3></a>
              </div>
        ...
        <div className="main">
          ...
          <div className="report-container">
            <div className="report-header">
              <h1 className="your-Schedule">Reminder</h1>
            ...
            <table id="scheduleTable" className="report-table">
              ...
              <tbody>
                {reminder && reminder.map((row, index) => (
                  <tr key={index}>
                    <td>{row.task}</td>
                    <td>{row.time}</td>
                  </tr>
